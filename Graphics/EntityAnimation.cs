...
namespace Battle_Cats_Ultimate_Test.Graphics
{
    ...
    public struct KeyFrame
    {
        ...
        public float DecidedFrame;//a float purely to support 60fps shenanigans.
        ...
        public void Decide()
        {
            DecidedFrame = BCUltimate.Does60FPS ? (Frame.GetRandom()*2f).StupidRound()*0.5f: Frame.GetRandom().StupidRound();
            DecidedValue = Value.GetRandom().StupidRound();
        }
    }
    public struct AnimModifier
    {
        ...
        public AnimModifier DeepCopy()
        {
            ...
            foreach (KeyFrame i in Frames) { newMod.Frames.Add(i.DeepCopy()); }
            return newMod;
        }
        public void NewKey()
        {
        ...
        public void SetCurrentValue(float frame)
        {
            ...
            {
                if (inLoopFrame < Frames[i].DecidedFrame) { ActedFrames.Append(i); }
                else
                {
                    ActedFrames.Append(i);
                    if (Frames[ActedFrames[0]].Ease != EasingType.Polynomic || Frames[i].Ease != EasingType.Polynomic) { break; }
                }
            }
            ...
            switch (Frames[ActedFrames[0]].Ease)
            {
                ...
                case EasingType.Exponential:
                    if (Frames[ActedFrames[0]].Parameter == 0) { time = time > 0f ? 1f : 0f; }
                    else
                    {
            ...
            if (Frames[ActedFrames[0]].Ease == EasingType.Polynomic)
            {
                ...
                foreach(int f1 in ActedFrames)
                {
                    ...
                    {
                        if(f1 != f2) { tempVal *= (time - Frames[f2].DecidedFrame)/(Frames[f1].DecidedFrame-Frames[f2].DecidedFrame); }
                    }
                    CurrentValue += tempVal;
                }
                CurrentValue = (CurrentValue/4096f).StupidRound();
            }
            else { CurrentValue = Frames[ActedFrames[0]].DecidedValue+(range*time).StupidRound(); }
        }
    }
    public class EntityAnimation
    {
        ...
        public EntityAnimation DeepCopy()
        {
            ...
            foreach(AnimModifier i in AnimModifiers) { newEntityAnim.AnimModifiers.Add(i.DeepCopy()); }
            return newEntityAnim;
        }
        public void NewModifier()
        {
            AnimModifiers.Add(new AnimModifier(0, ModIdentifier.PositionY, -1, ""));
        }
    }
}
